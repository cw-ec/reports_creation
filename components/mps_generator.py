...
class MPSGenerator:
    ...
    def gen_report_table(self) -> pd.DataFrame:
        ...
        apd_grouped = out_df[['PD_NO_CONCAT', 'ADV_PD_NBR']].groupby('PD_NO_CONCAT').agg(list).astype(str)
        apd_grouped.rename(columns={'ADV_PD_NBR':'APD_LIST'}, inplace=True)
        out_df = out_df.join(apd_grouped, on=['PD_NO_CONCAT'])
        out_df['APD_LIST'] = out_df['APD_LIST'].apply(lambda x: ', '.join(x) if isinstance(x, list) else '')



        print(out_df[['PD_NO_CONCAT', 'TOTAL_INST', 'TOTAL_ELECTORS', 'APD_LIST']].head())
        sys.exit()
